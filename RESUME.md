# Resume point — TechVet (2026-05-16 EOD-8 — priority #4 + round-2 A/B/G/J/L + K + K2 + Q + C + M shipped; round 4 validation complete; 4 hot patches + Fix O next)

**Status:** clean working tree. `npx tsc -b` clean; `npm test` passes **145/145**; `npx vite build` clean (1.24 MB / **~369 KB gzipped**). Ten things today:
...
**Recommended next: hot-patch round (~2.5h) + Fix O (~1d).** Round 4 validation surfaced 4 quick fixes that close half-shipped behavior in today's work:
- **Bug 1** — channel-aware copy for `Confirmed not in stack` section (1h). Push the `notDiscussedCopy` pattern through; closes Q's half-shipped loop.
- **Bug 2** — channel chip casing renders "ASYNC (CV-ONLY)" (10min). Drop the uppercase class on the channel chip.
- **Bug 3** — minimum query length OR no-results CTA on no-exact-match (1h). Single-letter search breaks Fix C's CTA for R/Go/Q.
- **Bug 5** — fractional-year placeholder + months formatter branch (30min). Sub-1-yr renders awkwardly.

Then **Fix O** (1d) — Bashir's source-verified recommendation: demote `langchain` `min: "1.0"` Green → Yellow, gate Green behind checklist services, same shape for `vector-db` and `llm-api-sdk`. Integrity test: "no AI/ML version-mode tech may have a Green tier without `releasedYear` within 18 months". This is the sharpest validation-confirmed urgent item from the round.

After O: Fix E (asymmetric `lastUsed`, 1.5d), Fix U (Security catalog, 1d), Bug 4 (extend `namedNotInCatalog` to `{name, depth?, lastUsed?}`, 1d).
