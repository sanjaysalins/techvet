# Resume point — TechVet (2026-05-16 EOD-5 — priority #4 + round-2 fixes A/B/G/J/L + K + K2 + Q shipped; Fix C next)

**Status:** clean working tree. `npx tsc -b` clean; `npm test` passes **125/125** (+6 channel tests today); `npx vite build` clean (1.24 MB / **~367 KB gzipped**). Seven things today:
...
**Recommended next: Fix C — named-not-in-catalog capture (1.5 days).** 4 round-3 sessions named it (Lou/Devon/Tomi/Pranav); cheaper than each catalog refresh and generalizes to all specialists. When search returns zero results, surface a "+ Add 'X' as named-only (not assessable)" button that lands the tech in a gray "Candidate mentioned — out of catalog" section on the PDF. Recruiter heard it, report shows it, hiring manager has a probe target. Closes Lou's Oracle ecosystem invisibility + Devon's 5 systems-prog vanishing + Tomi's Vault/Burp dead-end spiral + Dmitri's Ruby/Solidity vanishing.

After C: Fix M (candidate context block, 1d — 3 round-3 sessions confirmed urgency), then Fix E (asymmetric `lastUsed`, 1.5d). Fix O (broadened: fast-moving libs + AI/ML libs max at Yellow without checklist services) after that.
