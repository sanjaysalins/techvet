# Resume point — TechVet (2026-05-16 EOD-9 — priority #4 + round-2 A/B/G/J/L + K + K2 + Q + C + M + round-4 hot patches + Fix O all shipped)

**Status:** clean working tree. `npx tsc -b` clean; `npm test` passes **153/153** (+8 today); `npx vite build` clean (1.24 MB / **~369 KB gzipped**). Eleven things today:
...
**Recommended next: Fix E — asymmetric `lastUsed` in scoring (1.5d).** Sarah (round 3) + Dmitri (round 2) canonical cases. Per Sarah's design wrinkle: penalize stale Greens (Sam-Ansible from prior rounds) AND *soften* stale Reds when the version was contemporary at last-use (Sarah's Spring Boot 2.5 was current when she left in 2022). Parse `item.lastUsed` (free-text today: "current role", "2 years ago", "2022") in new `lib/lastUsed.ts` → coarse bucket. Add `versionTier.firstAvailable` (year) to tier shape so scorer can compute "was this version current when last used?". Plumb into `scoring.ts` after scope (order: tier → depth → scope → recency).

After E: Fix U (Security catalog overhaul, 1d — Wendy validates), Bug 4 (extend `namedNotInCatalog` to `{name, depth?, lastUsed?}` so depth signal isn't lost on captured names, 1d — Wendy proposed), role-aware AWS checklists (2d — 3 of 6 round-4 sessions confirmed urgent: Helena/Wendy/Owen), Backend template `techScopes` (0.5d — Owen).
