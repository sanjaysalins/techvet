# Resume point — TechVet (2026-05-16 EOD-10 — priority #4 + round-2 A/B/G/J/L + K + K2 + Q + C + M + round-4 hot patches + Fix O + Fix E all shipped)

**Status:** clean working tree. `npx tsc -b` clean; `npm test` passes **192/192** (+39 Fix E today); `npx vite build` clean (1.24 MB / **~370 KB gzipped**). Twelve things today:
...
**Recommended next: Fix U — Security catalog overhaul (1d).** Wendy round-4 session validated this is the unclosed gap K2 + C can't substitute for. Add Vault / Burp / Semgrep / Trivy / Snyk / Falco / OWASP ZAP / Grype as first-class catalog entries (some version-mode, some checklist-shaped for tools like Snyk that span scanners). Then swap or augment the Security template's preload to use them — currently the template preloads zero security tools.

After U: Bug 4 (extend `namedNotInCatalog` to `{name, depth?, lastUsed?}` — Wendy: "Burp daily, deep" flattens to bare "Burp" right now), role-aware AWS checklists (Helena/Wendy/Owen — 3 of 6 round-4 sessions confirmed urgent), Backend template `techScopes` (Owen), D4 methodology section (most-named open structural defect across all 4 rounds).
