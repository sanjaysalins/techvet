# Resume point — TechVet (2026-05-16 EOD-14 — priority #4 + round-2 + K + K2 + Q + C + M + round-4 hot patches + Fix O + Fix E + Fix U + Bug 4 + cloud defaultScope + AWS role-aware checklists all shipped)

**Status:** clean working tree. `npx tsc -b` clean; `npm test` passes **208/208** (+7 today); `npx vite build` clean (1.24 MB / **~373 KB gzipped**). Catalog: **103 entries** across 12 categories; AWS checklist 14 → 26 services with tags. Sixteen things today:
...
**Recommended next: D4 — methodology section (3-5d).** Most-named open structural defect across all 4 rounds (Mei R1, Marisol R3+R4, Yara R3). Senior DS/ML candidates are differentiated by methodology (DiD/IV/RDD/Bayesian/MCMC/causal inference), not tools — and the tool has no field for any of it. Three sub-design questions to settle before implementation: (a) per-role tag list (similar to AWS service tags but free-form), (b) new radar axis or just a section, (c) catalog-vs-recruiter-driven (curated per role, vs free-text recruiter tags). Recommend a design pass first — probably 3-4 sample personas across DS/ML/SRE to surface the right shape.

After D4: Remaining round-2/3 carryovers (R/S/T integrity-guard tweaks, ≤1d each).
