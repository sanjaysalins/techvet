# Resume point — TechVet (2026-05-16 EOD-11 — priority #4 + round-2 A/B/G/J/L + K + K2 + Q + C + M + round-4 hot patches + Fix O + Fix E + Fix U all shipped)

**Status:** clean working tree. `npx tsc -b` clean; `npm test` passes **195/195** (+3 Fix U today); `npx vite build` clean (1.24 MB / **~371 KB gzipped**). Catalog: 96 → **103 entries** across **12 categories** (Security category added). Thirteen things today:
...
**Recommended next: Bug 4 — extend `namedNotInCatalog` to `{name, depth?, lastUsed?}` (1d).** Wendy validated this: with the named-only structure being bare strings, "Burp daily, deep" still flattens to bare "Burp" even with Fix U adding Burp first-class. Useful for the long-tail vertical specialists (Lou-Oracle's PL/SQL, Devon's Tokio, etc.) where catalog-extension isn't economical. Migration via discriminated union (string OR object) preserves backward compat.

After Bug 4: role-aware AWS checklists (Helena/Wendy/Owen — 3 of 6 round-4 sessions confirmed urgent), Backend template `techScopes` (Owen, 0.5d), then D4 methodology section (3-5d, most-named open structural defect across all 4 rounds — likely warrants a design pass first).
