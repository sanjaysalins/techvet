# Resume point — TechVet (2026-05-16 EOD-6 — priority #4 + round-2 A/B/G/J/L + K + K2 + Q + C all shipped; Fix M next)

**Status:** clean working tree. `npx tsc -b` clean; `npm test` passes **135/135** (+10 named-only store tests today); `npx vite build` clean (1.24 MB / **~368 KB gzipped**). Eight things today:
...
**Recommended next: Fix M — candidate-context block on report (1 day).** 3 round-3 sessions confirmed urgency (Eitan: 8yr industry + 2yr coding internal transfer reads as junior; Riya: 6yr founder going IC reads as senior IC; Min: 4yr post-doc reads as 4yr industry MLE). Adds a structured `candidateContext` field on AssessmentMeta with seniority + years-in-industry + path-type (traditional / junior / returner / contractor / founder / academic / internal-transfer / OSS-maintainer) + free-text. Renders in the report header so hiring manager reads it before the verdicts.

After M: Fix E (asymmetric `lastUsed`, 1.5d — penalize stale Greens AND soften stale Reds when version was contemporary at last-use). Then Fix O (broadened: fast-moving libs + AI/ML libs max at Yellow without checklist services), Fix S (search aliases for collisions), Fix R (broaden Fix J integrity guard), Fix T (evaluated/migrated-off 4th tri-state).
