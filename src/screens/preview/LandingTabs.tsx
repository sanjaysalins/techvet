...
import { ArrowRight, FileSearch, FileText, ChevronLeft, Sparkles, Plus } from 'lucide-react';
...
/**
 * Preview B — Simple / Advanced tabs.
 * Default to Simple (JD textarea only). Advanced shows the template
 * grid + Resume Draft.
 */
...
interface ResolvedRow {
  ...
  name: string;
  category: string;
  matched?: string[]; // present if extracted; absent if added via search
}
...
function ExtractionResults({
  ...
  // Show everything currently selected — extracted (with evidence) + extras.
  // Also surface unselected extracted entries so the recruiter can re-tick.
  ...
  return (
    <div className="card p-6">
      ...
      {/* Add missing step — recruiter can search any catalog tech the JD
          mentioned generically ("AI solutions", "Cloud") or didn't name
          explicitly that they want assessed anyway. */}
